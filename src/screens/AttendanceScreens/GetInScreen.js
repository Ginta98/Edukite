...
import { QR_SCAN, LOGIN } from '../../navigator/RouteName'
import Icon from 'react-native-vector-icons/FontAwesome5';
import { size, texts } from '../../constant/CommonStyles';
import { scale, scaleModerate } from '../../constant/Scale';
import DropDownPicker from 'react-native-dropdown-picker';
import Loading from '../../components/Loading'
import { WHITE, GRAY_FONTCOLOR, PRIMARY_COLOR, BLACK, PURPLE_FONTCOLOR } from '../../constant/Colors';
import Message from '../../components/Message';
import { connect } from 'react-redux';
import { CommonActions } from '@react-navigation/native';
class GetInScreen extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            country: '',
    ...
    _onPressComplete() {
        ...
        if (dataStudentUnchecked.length !== 0) {
            this.setState({
    ...
    render() {
        if (this.props.scheduleInfo && this.props.listStops && this.props.nextStop) {
            if (this.props.scheduleInfo.code === 0 && this.props.listStops.code === 0 && this.props.nextStop.code === 0) {
                let scheduleInfo = this.props.scheduleInfo.data[0];
                let listStops = this.props.listStops.data;
                let dataStopDropDown = []
                listStops.map((item, index) => {
                    if (item.stopName) {
                ...
                return (
                    <View>
                        ...
                        <ScrollView
                            ...
                            <View style={{ flexDirection: 'row', width: '90%', height: scale(100) }}>
                                ...
                                <View style={{ width: '50%', height: '100%', justifyContent: 'center', alignItems: 'flex-start', marginLeft: scale(10) }}>
                                    ...
                                    >
                                        <Text style={[texts.h4, { color: PURPLE_FONTCOLOR, fontWeight: 'bold', textAlign: 'left' }]}>Lên xe: 49/50</Text>
                                    </View>
                                    <View
                                        style={{ marginBottom: scale(10) }}
                                    >
                                        <Text
                                            ...
                                            style={[texts.h4, { color: PURPLE_FONTCOLOR, fontWeight: 'bold', textAlign: 'left' }]}>Lên xe tại bến: 3/50</Text>
                                    </View>
                                    <View
                                    ...
                                    >
                                        <Text
                                            ...
                                            style={[texts.h4, { color: BLACK, textAlign: 'left' }]}>Xe {scheduleInfo.busId}</Text>
                                    </View>
                                </View>
                        ...
                        {this.state.openModalComplete ? <Message message={'Điểm danh thành công'} /> : null}
                    </View>
                )
            }
            else {
...
export default connect(mapStateToProps, mapDispatchToProps)(GetInScreen);
